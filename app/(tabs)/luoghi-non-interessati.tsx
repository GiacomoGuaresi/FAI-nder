...
export default function LuoghiNonInteressatiScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      (async () => {
        try {
          const notInterestedStored = await AsyncStorage.getItem(NOT_INTERESTED_STORAGE_KEY);
  ...
  return (
    <View style={styles.container}>
      {notInterestedProperties.length > 0 ? (
        <FlatList
          data={notInterestedProperties}
        ...
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptyText}>Nessun luogo non interessante</Text>
          <Text style={styles.emptySubtext}>Segna come non interessanti i luoghi che non vuoi visitare</Text>
        </View>
      )}
    </View>
  );
